...
- [x] **2D scenes.** Built 2026-09-24 ([[09 Decisions/Decisions#ADR-038 — 2D is a per-object motion plane, and picking and contacts speak both physics engines|ADR-038]]): picking and `when clicked` find 2D colliders and bare sprites, 2D contacts reach `when collided` and `touching?`, and motion runs on the XY stage for sprites. *Confirm in Unity:* a small 2D scene (a sprite with a `BoxCollider2D` and a `Rigidbody2D`, an orthographic camera) — pick it, move and turn it, bump it into a tagged sprite, Reset.
