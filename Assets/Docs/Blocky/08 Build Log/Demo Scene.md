...
- **Palette** — visible as soon as the workspace opens, so you can browse it before picking an object (dragging from it starts working once an object is selected). Every block drawn in its real shape: events are hats (rounded top, nothing can go above), ordinary blocks have a notch on top and a tab underneath (connect both ways), C-blocks have a mouth (`if`, `repeat`; `if else` has two), and `forever` is a cap (no tab — nothing can follow it).
