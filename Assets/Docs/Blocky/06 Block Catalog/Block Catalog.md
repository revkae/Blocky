...
**For a game's own blocks: `Blocky › New Block…`** ([[09 Decisions/Decisions#ADR-036 — New blocks live in the project's own folder, and a missing op stops one script, not the game|ADR-036]]). It asks for the name, the kind (command, value or question), the palette tab and the inputs, and makes the asset, a class with every input already read, and the words in every language — in a folder of the project's own (`Assets/BlockyBlocks` by default), so updating Blocky never touches them. The op is found in any assembly that references Blocky.Runtime, and a game's op for a built-in block type replaces Blocky's. Hats and C-blocks are still made by hand: a hat needs the runner to know its event, a C-block the VM's frames.
