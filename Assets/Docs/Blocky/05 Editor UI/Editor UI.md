...
- **Drag** — `ChainDragSession` (ghost, snap evaluation, glow, one `DropChain` on release), driven by `PaletteDragManipulator` (new blocks) and `CanvasDragManipulator` (existing blocks; grab takes the tail, a hat takes the stack, drop on palette deletes). Pointer tracked on the panel root with TrickleDown, not capture — reparenting into the drag layer would drop the capture.
...
**Still not done:** the Editor window (`Blocky/Program Editor`) still edits structure with buttons — it renders the new shapes but has no drag. Expression/Reporter param editing remains a v1 non-goal (unimplemented placeholder label).
